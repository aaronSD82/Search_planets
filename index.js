...
async function launchQuery(offsetSet) {
  ...
  try {
    if (
      ...
      inputMaxMass.value ||
      inputMinMass.value ||
      inputMaxRadius.value ||
      inputMinRadius.value ||
      inputMaxPeriod.value ||
      inputMinPeriod.value ||
      inputMaxDistance.value ||
      ...
      offset = 0;
      otherPlanet.innerHTML =
        "<h1>Debe introducir algun campo de búsqueda</h1>";
      otherPlanet.style.display = "flex";
    }
  } catch (error) {
    console.log(error);
...
function madeCompleteRequest(completeQuery) {
  ...
  let radiusMin = inputMinRadius.value
    ? `&min_radius=${inputMinRadius.value}`
    : "";
  let radiusMax = inputMaxRadius.value
    ? `&max_radius=${inputMaxRadius.value}`
    : "";
  let periodMin = inputMinPeriod.value
    ? `&min_period=${inputMinPeriod.value}`
    : "";
  let periodMax = inputMaxPeriod.value
    ? `&max_period=${inputMaxPeriod.value}`
    : "";
  let tempMin = intputMinTemp.value
    ? `&min_temperature=${intputMinTemp.value}`
    : "";
  let tempMax = inputMaxTemp.value
    ? `&max_temperature=${inputMaxTemp.value}`
    : "";
  let distanceMin = inputMinDistance.value
    ? `&min_distance_light_year=${inputMinDistance.value}`
    : "";
  let distanceMax = inputMaxDistance.value
    ? `&max_distance_light_year=${inputMaxDistance.value}`
    : "";
